...
def parse_stacks(stacks: list):
    s1, s2, s3, s4, s5, s6, s7, s8, s9 = ([] for _ in range(9))  # type: ignore

    # first element in list will be the top, last element the bottom of the stack
...
def part_1(file_path: str) -> str:
    ...
    for line in procedure:
        ...
        for i in range(n):
            stacks_list[from_stack_n - 1], stacks_list[to_stack_n - 1] = move_top(
                stacks_list[from_stack_n - 1], stacks_list[to_stack_n - 1]
            )
...
def part_2(file_path: str) -> str:
    ...
    for line in procedure:
        n, from_stack_n, to_stack_n = parse_procedure_line(line)
        stacks_list[from_stack_n - 1], stacks_list[to_stack_n - 1] = move_top_n(
            n, stacks_list[from_stack_n - 1], stacks_list[to_stack_n - 1]
        )
